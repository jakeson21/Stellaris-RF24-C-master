...
#if defined(TI_ARM)
	uint32_t ssi_base;
	uint32_t ce_pin_port_base;
	uint32_t csn_pin_port_base;
#endif
} tRF24;
...
void RF24_init_speed(tRF24* rf24, uint8_t _cepin, uint8_t _cspin,
		uint32_t spispeed);
//#endif
...
void RF24_openReadingPipe(tRF24* rf24, uint8_t number, const uint8_t *address);
...
void RF24_startFastWrite(tRF24* rf24, const void* buf, uint8_t len,
		const bool multicast, bool startTx);
 ...
 */
void RF24_maskIRQ(tRF24* rf24, bool tx_ok, bool tx_fail, bool rx_ready);
 ...
 */
//void RF24_openReadingPipe(tRF24* rf24, uint8_t number, uint64_t address);
/**
 ...
 * @param address The 40-bit address of the pipe to open.
 */
//void RF24_openWritingPipe(tRF24* rf24, uint64_t address);
//private:
/**
 ...
 * @return Current value of status register
 */
uint8_t RF24_write_payload(tRF24* rf24, const void* buf, uint8_t len,
 ...
 * @return Current value of status register
 */
uint8_t RF24_read_payload(tRF24* rf24, void* buf, uint8_t len);
